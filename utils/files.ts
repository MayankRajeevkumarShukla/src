...
export async function readFile(filePath: string): Promise<string | null> {
  try {
    ...
    logger.debugLog(`Failed to read file: ${filePath}`, err as Error);
    return null;
  }
}
...
export async function writeFile(filePath: string, content: string): Promise<void> {
  try {
    ...
    logger.debugLog(`Failed to write file: ${filePath}`, err as Error);
  }
}
 ...
 * @returns {string | null}
 */
export function getShellHistoryPath(shell: string): string | null {
